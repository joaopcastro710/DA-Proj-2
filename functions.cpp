...
std::vector<std::vector<double>> calculateShortestPaths(Graph<int>& graph, const std::unordered_set<int>& nodes) {
    ...
    std::vector<std::vector<double>> dist(numV, std::vector<double>(numV, std::numeric_limits<double>::max()));

    for (auto node : nodes) {
        std::priority_queue<std::pair<double, Vertex<int>*>, std::vector<std::pair<double, Vertex<int>*>>, std::greater<>> priorityQueue;
...
Graph<int> formMST(Graph<int> g, int currentNode) {
    ...
    // adiciona os vértices da MST
    for (auto v : g.getVertexSet()) {
        mst.addVertex(v->getInfo());
    }

    // ligar arestas da MST
    for (auto v : g.getVertexSet()) {
        if (v->getInfo() != currentNode) {
            auto edge = v->getPath();

            // Verifica se a edge não é nula senão dá erro
            if (edge != nullptr) {
...
Graph<int> MinWeightPerfMatch(Graph<int>& graph, const std::vector<int>& odders) {
    Graph<int> grafo;
    for(int odd : odders) { // para cada vertice
        stack<Vertex<int>> pilha;
        auto v = graph.findVertex(odd);
        if(grafo.findVertex(v->getInfo()) == nullptr) {
            grafo.addVertex(v->getInfo());
        }
        if(!v->isVisited()) { // se não visitado
            v->setVisited(true); // visitar
            int min = INT_MAX;
            for(auto e : v->getAdj()) { // ver na lista de arestas adjacentes
                if(isOdd(*e->getDest()) && !e->getDest()->isVisited()) { // so vai se não tiver sido visitada e for odd
                    if(e->getWeight() < min) { // se o peso dessa aresta for o menor
                        if(!pilha.empty()) pilha.pop(); // tira da stack para atualizar o novo peso
                        pilha.emplace(e->getDest()->getInfo()); // atualiza com o novo peso
                    }
                }
            }
            if(!pilha.empty()){
                grafo.addVertex(pilha.top().getInfo());
                grafo.addEdge(v->getInfo(), pilha.top().getInfo(), getEdgeWeight(graph, v->getInfo(), pilha.top().getInfo()));
                graph.findVertex(pilha.top().getInfo())->setVisited(true); // visitar
            }}
    }
...
vector<Vertex<int>*> findEulerianPath(Graph<int>& graph) {
    ...
    while (!currPath.empty()) {
        ...
        if (u->getAdj().empty()) {
            ...
            graph.removeEdge(v->getInfo(), u->getInfo()); // grafo nao direcionado
        }
    }
...
Graph<int> combineGraphs(const Graph<int>& mst, const Graph<int>& matching) {
    ...
    // Adiciona arestas entre vertices odd commin weight à MST
...
vector<Vertex<int>*> christofidesTsp(Graph<int>& graph) {
    ...
    // Passo 2: Identificar os vértices odd
    ...
    // Passo 5: Encontrar um Euler Path
    auto eulerianPath = findEulerianPath(eulerianMultigraph);

    // Passo 6: Transformar Euler Path num HamiltonianCycle
